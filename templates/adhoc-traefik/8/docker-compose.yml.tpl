...
services:
    traefik:
        ...
        ports:
            ...
            - ${http_port}:80/tcp
            - ${https_port}:443/tcp
        # si no funca con "--acme.httpChallenge.entryPoint=" podemos probar --acme.httpchallenge=false (idem --acme.dnschallenge)
        {{- if and (eq .Values.acme_http_challenge "false") (ne .Values.acme_dns_challenge "gcloud")}}
        command: sh -c "traefik storeconfig --api.dashboard --checknewversion=false --rancher --rancher.domain=${domain} --rancher.metadata --rancher.enableservicehealthfilter=${EnableServiceHealthFilter} --logLevel=INFO --defaultentrypoints=http,https --insecureskipverify=true '--entryPoints=Name:http Address::80 Redirect.EntryPoint:https' '--entryPoints=Name:https Address::443 TLS Compress:on' --acme --acme.acmelogging --acme.entrypoint=https --acme.httpChallenge.entryPoint= --acme.dnschallenge.provider= --acme.email=${acme_email} --acme.onhostrule=${acme_onhostrule} --acme.storage=traefik/acme/account --consul.endpoint=adhoc-consul-consul-1:8500 && traefik --consul --consul.endpoint=adhoc-consul-consul-1:8500"
        {{- end}}
        {{- if eq .Values.acme_http_challenge "true"}}
        command: sh -c "traefik storeconfig --api.dashboard --checknewversion=false --rancher --rancher.domain=${domain} --rancher.metadata --rancher.enableservicehealthfilter=${EnableServiceHealthFilter} --logLevel=INFO --defaultentrypoints=http,https --insecureskipverify=true '--entryPoints=Name:http Address::80 Redirect.EntryPoint:https' '--entryPoints=Name:https Address::443 TLS Compress:on' --acme --acme.acmelogging --acme.entrypoint=https --acme.email=${acme_email} --acme.onhostrule=${acme_onhostrule} --acme.storage=traefik/acme/account --acme.httpChallenge --acme.httpChallenge.entryPoint=http --acme.dnschallenge.provider= --consul.endpoint=adhoc-consul-consul-1:8500 && traefik --consul --consul.endpoint=adhoc-consul-consul-1:8500"
        {{- end}}
        {{- if eq .Values.acme_dns_challenge "gcloud"}}
        command: sh -c "traefik storeconfig --api.dashboard --checknewversion=false --rancher --rancher.domain=${domain} --rancher.metadata --rancher.enableservicehealthfilter=${EnableServiceHealthFilter} --logLevel=INFO --defaultentrypoints=http,https --insecureskipverify=true '--entryPoints=Name:http Address::80 Redirect.EntryPoint:https' '--entryPoints=Name:https Address::443 TLS Compress:on' --acme --acme.acmelogging --acme.entrypoint=https --acme.email=${acme_email} --acme.onhostrule=${acme_onhostrule} --acme.storage=traefik/acme/account --acme.dnschallenge --acme.domains=*.${domain} --acme.httpChallenge.entryPoint=  --acme.dnschallenge.provider=gcloud --consul.endpoint=adhoc-consul-consul-1:8500 && traefik --consul --consul.endpoint=adhoc-consul-consul-1:8500"
        {{- end}}
