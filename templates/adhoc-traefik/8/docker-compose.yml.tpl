...
services:
    traefik:
        ...
        ports:
            ...
            - ${https_port}:443/tcp
            # log_driver: ''
        command: sh -c "traefik storeconfig --api.dashboard --checknewversion=false --rancher --rancher.domain=${domain} --rancher.metadata --rancher.enableservicehealthfilter=${EnableServiceHealthFilter} --logLevel=INFO --defaultentrypoints=http,https --insecureskipverify=true '--entryPoints=Name:http Address::80 Redirect.EntryPoint:https' '--entryPoints=Name:https Address::443 TLS Compress:on' {{- if eq .Values.acme_enable "true"}}--acme --acme.acmelogging --acme.entrypoint=https --acme.email=${acme_email} --acme.onhostrule=${acme_onhostrule}{{- end}} {{- if eq .Values.acme_http_challenge "true"}--acme.httpChallenge.entryPoint=http{{- end}} {{- if eq .Values.acme_dns_challenge "route53"}}--acme.dnschallenge --acme.domains=*.${domain} --acme.dnschallenge.provider=route53{{- end}} {{- if eq .Values.acme_dns_challenge "gcloud"}}--acme.dnschallenge --acme.domains=*.${domain} --acme.dnschallenge.provider=gcloud{{- end}} --consul.endpoint=${ConsulEndpoint} && traefik --consul --consul.endpoint=${ConsulEndpoint}"
